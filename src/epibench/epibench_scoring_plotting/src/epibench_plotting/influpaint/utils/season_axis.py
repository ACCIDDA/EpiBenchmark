...
INFLUPAINT_DIR = Path(__file__).resolve().parents[1]
...
class SeasonAxis:
    ...
    - season_start_month (int, optional): Start month for flu seasons. Defaults to class attribute value.
    - season_start_day (int, optional): Start day for flu seasons. Defaults to class attribute value.

    Class Attributes: # NEW addition, include two class attributes so they can be directly used in other modules.
    - season_start_month (int): Start month for flu seasons (1-12, Defaults to 8, i.e. August)
    - season_start_day (int): Start day for flu seasons (1-31, Defaults to 1)
    ...
    def __init__(
        ...
        print(f"Spatial Setup with {len(self.locations_df)} locations, with a season start_date of {pd.to_datetime(f'2020-{self.season_start_month:02d}-{self.season_start_day:02d}').strftime('%b %d')}")
