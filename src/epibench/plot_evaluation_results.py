...
from epibench_plotting.influpaint.utils.season_axis import SeasonAxis
from epibench_plotting.benchmark_plotting import plot_components, plot_timeseries, plot_wis_heatmap, plot_cumulative_timeseries, plot_multi_location_stacked, print_ladderboard, compute_missing_data, get_rankings, plot_interactive_model_selection

# %% Configuration
CSV_PATH = PROJECT_ROOT/"results"/"EpiBench_scores_enhanced.csv" #add column group to the original EpiBench_scores.csv
SAVE_DIR = PROJECT_ROOT/"results"/"simple_plots"
GROUP_COLORS = {'influpaint': 'green', 'flusight': 'blue'}
ALLOW_MISSING_DATES_PER_MODEL = 5  # Same threshold as evaluation_pipeline.py
LEADERBOARD_DIR = PROJECT_ROOT/"results"/"leaderboards"
...
def get_missing_data_for_plot(original_df: pd.DataFrame, models_in_plot: List[str], location_filter: str, season_filter: str = None) -> Dict[str, Dict[str, Union[str, bool]]]:
    ...
    jobs_file = PROJECT_ROOT/"paper_runs_2025-07-22"/"inpaint_jobs_paper-2025-07-22.txt"
    try:
...
if __name__ == "__main__":
    ...
    df_raw['target_end_date'] = pd.to_datetime(df_raw['target_end_date']) #convert the column to pandas datetime instead of standard python date object
    df_raw['location'] = df_raw['location'].astype(str).str.strip()

    print('df raw date type:', df_raw.dtypes, df_raw['target_end_date'].dtype)

    # NEW addition: create flu season column
    # Extract year and month once
    target_end_year = df_raw["target_end_date"].dt.year
    ...
    print('add column season\n', df_raw.head(2))
    
    # NEW addition: Add reference_date column to df_raw
    # define the multiplier
    time_units_per_horizon = 7 # 7 days
    
    # create reference_date
    df_raw['reference_date'] = (df_raw['target_end_date'] - pd.to_timedelta(df_raw['horizon'] * time_units_per_horizon, unit='D')) #object
    ...
    print('seasons to plot:', seasons_to_plot) #debug
    ...
    for season in seasons_to_plot:
        ...
        season_axis = SeasonAxis.for_flusight(remove_us=True, remove_territories=True)
        print('season axis\n', season_axis) #debug
        print('=== heatmap plotting in evaluation.py ===') #debug
        print('season_df\n', season_df.head(2)) #debug
        print('season raw df\n', season_raw_df.head(2)) #debug
        ...
        if not us_ts_data.empty:
            ...
            if 'group' in us_ts_data.columns:
                from epibench_plotting.benchmark_plotting import get_top_models_per_group
